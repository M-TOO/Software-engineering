...
require_once 'api_db_config.php';<?php
/**
 * CAASP Admin Dashboard
 * * Manages business approvals, user accounts, and content moderation.
 */
...
// !! IMPORTANT: REMOVE THIS BLOCK BEFORE DEPLOYMENT !!
$user_id = 1;    // Mock Admin User ID
$role = 'Admin'; // Mock Admin Role

/* // --- ORIGINAL AUTHENTICATION CHECK (DISABLED FOR VIEWING) ---
$user_id = $_SESSION['user_id'] ?? null;
$role = $_SESSION['role'] ?? null;

// Ensure user is logged in AND is an Admin (Assuming you log in as Admin)
...
function fetch_all_listings($db) {
    // This example fetches the last 20 services and parts
    $listings = [];
    $sql_services = "
        SELECT S.service_id AS item_id, S.service_name AS item_name, S.service_price AS price, 'Service' AS type, G.garage_name AS business_name
        FROM Services S JOIN Garages G ON S.garage_id = G.garage_id
        ORDER BY S.service_id DESC LIMIT 10
    ";
    $sql_parts = "
        SELECT P.part_id AS item_id, P.part_name AS item_name, P.part_price AS price, 'Part' AS type, V.vendor_name AS business_name
        FROM Parts P JOIN Vendors V ON P.vendor_id = V.vendor_id
        ORDER BY P.part_id DESC LIMIT 10
    ";

    // Combine results (basic approach)
...
// Data fetching proceeds normally, even with the bypass, but requires DB connection
if ($db) {
